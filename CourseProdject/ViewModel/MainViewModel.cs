        ...
        public async Task LoadData()
        {
            ...
            while (true)
            {
                ...
                var groupedData = noGroupProcessesList
                    .GroupBy(obj => new { obj.Name, obj.Description }) // Группировка по Name и Description
                    .Select(group => new ProcessInfo(
                        group.Key.Name,
                        group.Sum(obj => obj.PidProcess),
                        group.Sum(obj => obj.CpuUsage),
                        group.Sum(obj => obj.MemoryUsage),
                        group.Key.Description)) // Использование Key.Description для описания
                    .ToList();
